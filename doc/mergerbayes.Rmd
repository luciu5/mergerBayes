...
We run our Bayesian Model Comparison on this single-market dataset using a **Strict Structural Specification** (mimicking standard antitrust calibration). We remove the supply intercept to force $\alpha$ to match the margin levels, and we use random effects to fit demand shares.

The results below show the comparison. Note that while **Monopolistic Competition** is favored, the overall fit is constrained by the tension between the margin-implied elasticity and the observed price-share correlations.

```{r pnb_results, echo=FALSE, message=FALSE, warning=FALSE}
# Load Model Results
pnb_res <- read_csv("../results/pnb_model_results_tight.csv", show_col_types = FALSE) %>%
  mutate(bayes_factor_vs_best = tryCatch(exp(LogML - max(LogML, na.rm = TRUE)), error = function(e) NA)) %>%
  select(Model, LogML, LOOIC, RMSE_Share, Alpha) %>%
  rename("RMSE" = RMSE_Share)
