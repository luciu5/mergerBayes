functions {
  ...
  // ---------------------------------------------------------------------------
  // 2. OPTIMIZED JOINT LIKELIHOOD FUNCTION (DECOUPLED)
  // ---------------------------------------------------------------------------
  real partial_sum_fast(array[] int slice_idx,
    ...
    for (n in 1:N_slice) {
      ...
      // --- C. ACCUMULATE (DECOUPLED + ABSOLUTE) ---
      
      // 1. DEMAND: Absolute Error on Shares
      // We predict 'mu1' (which is log-scale). We convert to exp(mu1) to match raw shares.
      // sigma_share_abs is passed 
      lp += normal_lpdf(shareIn[i] | exp(mu1), sigma_share_abs); 

      // 2. SUPPLY: Independent Margin Error
      // No conditional logic. No slope. Just fit the margins.
...
model {
  ...
  s0_offset ~ normal(0, 2.0); // Relaxed prior
  ...
  // DECOUPLED PRIORS
  sigma_share_abs ~ normal(0, prior_sigma_share); // Absolute sigma prior
  ...
  target += reduce_sum(
    ...
    sigma_share_abs, sigma_margin,  // Pass absolute sigma
    rateDiff_sd,
